...
| 编号 | 状态 | 负责人 | 工作模块 | 目标与具体任务 | 当前基础与剩余缺口 | 交付与验收标准 | 依赖/关联 |
|---|---|---|---|---|---|---|---|
| TASK-20260711-12 | 待开始 | 待领取 | Agent 增强分析能力 | 增加 `diagnose_congestion`、`detect_signal_anomaly`、`detect_spillback_risk`、`compare_strategy_metrics`、`draft_emergency_dispatch`、`generate_daily_operation_report` 等分析工具。 | 已有交通领域系统提示词和聊天界面，但分析结论主要由模型根据摘要生成，缺少基于工具证据的诊断、引用和可审计输出。工具分层、优先级和阶段计划已整理到 `backend/docs/AGENT_TASK_PLAN.md`。 | 支持拥堵诊断、异常分析、策略解释和日报生成；每条结论能追溯到工具结果或数据库记录；建议类工具默认不直接执行控制动作。 | 依赖 TASK-06、TASK-11；应急草案遵守 TASK-01、TASK-02。 |
