...
| TASK-20260712-03 | 待验证 | Codex | 数据分析仿真实时化 | 将本次累计通行量、当前平均排队/等待、路网健康评分、状态分布和近期路口监测改为读取当前仿真会话内存缓存；新会话清零，策略优化后健康评分和拥堵分布改善；同步修正历史日期与昨日统计文案。 | 已移除数据分析页对通行量、排队、等待、健康评分、拥堵分布和监测表的随机定时更新；Pinia 按真实帧维护会话级监测缓存，Dashboard 与数据分析页可接续同一 `sid`，新建或切换策略时统一清零。剩余真实 CityFlow 页面往返与策略切换人工联调。 | 新仿真开始时累计通行量和监测缓存为空；收到真实帧后指标与监测表实时更新；畅通路口保持多数，MaxPressure/Traffic-R 等策略健康评分稳定在 90 分以上；日期为当天之前 12 天；前端类型检查、相关 Vitest 与构建通过。 | 影响 `traffic.ts` store、`Dashboard.vue`、`DataAnalysis.vue` 及其测试；复用现有 WebSocket 与 CityFlow 帧，不新增后端协议。 |
