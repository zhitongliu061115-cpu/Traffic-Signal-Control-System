...
| TASK-20260714-01 | 进行中 | Codex | 济南地图轨迹保持 | 修复“济南切换到杭州、再切回济南并启动仿真”时，仿真路网重载覆盖高德真实道路折线的问题。实施步骤：复现并锁定覆盖点；同一济南展示拓扑重载时保留现有 `road.path`；补充场景切换回归测试；执行前端类型检查、单测和构建。 | 已确认 `initSimulationSession()` 会再次调用 `loadSceneRoadnet()`，而济南分支会重新克隆展示路网，覆盖场景切换后已经规划完成的真实路径。 | 济南→杭州→济南后，启动仿真前后的每条济南道路路径保持不变且不退化为端点直线；杭州首次切回济南时仍能正确恢复济南拓扑；相关自动化验证通过。 | 影响 `sys-frontend/src/stores/traffic.ts`、`sys-frontend/src/stores/__tests__/traffic.spec.ts`；不改变接口、部署或 DTO。 |
