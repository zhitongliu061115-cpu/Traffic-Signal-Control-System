...
| `GET` | `/api/v1/data-analysis/bootstrap` | 无 | `DataAnalysisBootstrapResponse` | 从数据库一次性返回数据分析页初始快照，包括指标卡及趋势、状态分布、日/小时序列、路口汇总、热力图、构成图、散点图、策略对比、监测记录、toast、实时游标和轮询间隔。 |
...
| `GET` | `/api/v1/data-analysis/forecast` | 无 | `ForecastResponse` | 读取 12 个路口最近 30 个连续分钟并调用独立预测服务，返回每路口 +10 分钟结果和路网 +2/+4/+6/+8/+10 分钟时间线。 |
...
`ForecastResponse.available=false` 表示模型关闭、历史窗口不足、路口覆盖不完整或模型服务不可达；前端必须展示不可用状态，禁止回退到固定数组、随机数或正弦波。可用响应必须携带 `modelVersion`、`modelType`、`dataUntil`、`trainedSource`、`intersections` 和 `timeline`。`trainedSource` 含 `SYNTHETIC` 时，页面必须明确标记为合成训练集。
