...
当前默认联调方式是：Spring Boot 与短时预测服务在本地运行，Python CityFlow 在阿里云 24 小时运行，Traffic-R 在 AutoDL 按需启动。
...
本地 Spring Boot
  -> 本地 traffic-forecast :17008
  -> 云端 PostgreSQL 历史交通数据
```
...
短时预测位于 `cloud/traffic-forecast`，与 Traffic-R 相位控制模型是两个独立进程。首次安装、合成数据生成和训练命令见该目录 `README.md`。已有模型制品时启动：

```powershell
cd cloud/traffic-forecast
...
Invoke-RestMethod http://127.0.0.1:8080/api/v1/data-analysis/forecast
```
...
```yaml
traffic-forecast:
  enabled: ${TRAFFIC_FORECAST_ENABLED:true}
  base-url: ${TRAFFIC_FORECAST_BASE_URL:http://127.0.0.1:17008}
  predict-path: /predict
  lookback-minutes: 30
  expected-intersections: 12
```

模型权重位于被 Git 忽略的 `cloud/traffic-forecast/models`。更换机器或部署环境必须在目标环境训练或分发已验收制品；不能只部署代码而假定本地权重存在。
...
后续验收或演示如果把 Spring Boot、Python CityFlow、Traffic-R、短时预测模型和 PostgreSQL 都部署在同一台 Linux 云服务器上，推荐采用“五个独立进程 + 仅 Spring Boot 对外暴露”的方式：
...
|-- Traffic Forecast:  127.0.0.1:17008 仅 Spring Boot 访问
...
4. 启动短时预测服务，确认其模型制品和数据库连接配置有效。
5. 在服务器本机验证 `curl http://127.0.0.1:9000/health`。
6. 在服务器本机验证 `curl http://127.0.0.1:6008/health` 和 `POST /predict-batch`。
7. 在服务器本机验证 `curl http://127.0.0.1:17008/health`。
8. 启动 Spring Boot，并通过环境变量覆盖：

```sh
export CITYFLOW_BASE_URL=http://127.0.0.1:9000
export TRAFFIC_R_BASE_URL=http://127.0.0.1:6008
export TRAFFIC_FORECAST_BASE_URL=http://127.0.0.1:17008
...
| 短时预测模型 | `curl http://127.0.0.1:17008/health` | 返回激活的 `modelVersion` 和 15 个模型 |
| 短时预测后端链路 | `GET /api/v1/data-analysis/forecast` | `available=true`、12 个路口和 5 个时间点 |
...
- 当前短时预测服务同样使用 `ThreadingHTTPServer`；生产部署应置于进程管理器和内部网络中，并监控模型加载、历史窗口完整性与推理延迟。
- 当前已训练版本使用明确标记的合成数据，只能证明训练与推理链路，不代表真实道路预测精度。真实采集数据达到验收跨度后必须使用 `--real-only` 重训和回测。
