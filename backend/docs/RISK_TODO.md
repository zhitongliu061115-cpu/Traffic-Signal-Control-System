...
| RISK-027 | P1 | backend/sim-python/session | 多个仿真 session 并行时，每个 CityFlow Engine 都会占用独立 worker 和 CPU | 高倍速或 stress 场景并发过多会降低所有会话帧率 | 已处理 | 新建会话不再因达到 `SIM_MAX_ACTIVE_SESSIONS` 返回 429；通过 stop、自然结束、`SIM_SESSION_IDLE_TTL_SECONDS`、`SIM_SESSION_ABANDONED_TTL_SECONDS`、`SIM_SESSION_MAX_LIFETIME_SECONDS` 和后台清理线程释放 Engine、worker、EV 状态与后端策略状态 |
...
| RISK-029 | P1 | deployment/performance | 云端服务器同时运行多个高倍速 CityFlow session 时，CPU 和响应时间会急剧恶化 | 前端帧率下降、策略下发延迟、多人共享体验变差 | 已处理 | `SIM_MAX_ACTIVE_SESSIONS` 仅作为软配置展示，创建请求不再按数量拒绝；继续使用 `SIM_MAX_SPEED`、`SIM_VISIBLE_VEHICLE_LIMIT`、`SIM_MAX_REQUEST_BYTES`、idle TTL 和最大生存期控制资源占用 |
...
| RISK-032 | P2 | backend/agent/mcp | 已确认当前不再接入百炼平台 Agent/MCP 工具注册，`/api/v1/agent/chat` 统一由后端自建 Agent 编排 | 旧 MCP 接入风险不再作为当前实现路径；如果未来重新暴露工具给外部平台，仍需补充鉴权、参数 schema 和只读边界 | 不适用 | 保留 `/api/v1/agent/tools/**` 作为后端内部/调试查询入口；外部开放前必须重新评审鉴权和公网访问边界 |
| RISK-033 | P2 | backend/agent/audit | Agent 工具调用审计已由后端自建编排层统一创建会话、消息和 `agent_tool_call`，不再依赖百炼 MCP 适配层传递 `messageId` | 旧平台适配链路风险已废弃；当前主要风险转为后端编排层是否完整记录 LLM 规划、工具参数、结果摘要和异常 | 已缓解 | 继续禁止保存 API Key、认证头或过大的结果 payload；联调时重点检查 `/api/v1/agent/chat` 的审计链路是否完整 |
