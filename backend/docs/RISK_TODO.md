...
| RISK-015 | P2 | backend/strategy | Spring Boot 已通过 `StrategyDispatchService` 统一生成 Fixed-Time、Max-Pressure、Traffic-R 决策，并通过 Python `/cityflow/simulations/{sid}/actions` 下发；但当前流程是先取 frame 再下发 actions | 信号控制效果通常从下一帧开始体现，前端如果逐帧比对可能误以为当前帧没有响应策略 | 已处理 | 已新增 Python actions 接口和 Spring Boot `CityFlowClient.applyControlActions`；后续可增加 `control.decision` WebSocket 消息或在下一帧标记上一次应用的决策 |
