...
| RISK-027 | P1 | backend/sim-python/session | 高车流 stress 场景下多个仿真 session 同时存在会让多个 CityFlow Engine 后台 step，并让 Spring 轮询多个 RUNNING session | 旧 session 未销毁时会抢占 CPU，导致新建仿真帧率下降、策略日志混杂、前端观感不稳定 | 已处理 | 当前联调限制为单活跃 session：Spring 创建新仿真前停止并清空旧 session；Python 创建新 session 前停止旧 worker 并清空旧 Engine session。后续多用户并发需要重做 session 隔离和资源配额 |
| RISK-028 | P1 | deployment/sim-python | CityFlow 准备部署到阿里云公网，多人开发如果共用单活 session 会互相清理仿真；如果没有访问令牌，`/cityflow/**` 会公网裸奔 | 团队成员联调时会互相踢掉 session，公网端口也可能被未授权请求调用 | 已处理 | Python CityFlow 增加 `CITYFLOW_API_TOKEN`，Spring Boot 增加 `cityflow.api-token`；会话改为按 `X-CityFlow-Client` 隔离，同一 client 只清理自己的旧 session |
