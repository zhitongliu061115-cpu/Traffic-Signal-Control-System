...
export const useTrafficStore = defineStore('traffic', () => {
  ...
  const intersections = ref<Intersection[]>(
    structuredClone(mockIntersections).map(it => ({ ...it, queueLength: 0, averageDelay: 0, congestionIndex: 0 }))
  )
  const roads = ref<Road[]>(
    structuredClone(mockRoads).map(r => ({ ...r, congestionIndex: 0, queueLength: 0, flow: 0, speed: 0 }))
  )
  ...
  function resetAllData(): void {
    intersections.value = structuredClone(mockIntersections).map(it => ({ ...it, queueLength: 0, averageDelay: 0, congestionIndex: 0 }))
    roads.value = structuredClone(mockRoads).map(r => ({ ...r, congestionIndex: 0, queueLength: 0, flow: 0, speed: 0 }))
    vehicles.value = []
