...
import type { SimLaneLink, SimRoadnetResponse, SimRoadnetRoad, SimVehicleState } from '@/types/traffic'
...
interface LocalVehiclePoseContext {
  coordinates: IntersectionCoordinateSystem
  connectedRoads: Map<string, SimRoadnetRoad>
  laneLinks: Map<string, { laneLink: SimLaneLink; movement: VehicleMovement }>
  approachMovements: Map<string, VehicleMovement[]>
}

const poseContexts = new WeakMap<SimRoadnetResponse, Map<string, LocalVehiclePoseContext | null>>()

function approachLaneKey(roadId: string, laneIndex: number): string {
  return `${roadId}:${laneIndex}`
}

function getPoseContext(
  roadnet: SimRoadnetResponse,
  intersectionId: string,
): LocalVehiclePoseContext | null {
  let contexts = poseContexts.get(roadnet)
  if (!contexts) {
    contexts = new Map()
    poseContexts.set(roadnet, contexts)
  }
  if (contexts.has(intersectionId)) return contexts.get(intersectionId) ?? null

  const intersection = roadnet.intersections.find((item) => item.id === intersectionId)
  if (!intersection) {
    contexts.set(intersectionId, null)
    return null
  }
  ...
  const laneLinks = new Map<string, { laneLink: SimLaneLink; movement: VehicleMovement }>()
  const approachMovements = new Map<string, VehicleMovement[]>()
  for (const roadLink of roadnet.roadLinks) {
    if (roadLink.intersectionId !== intersectionId) continue
    const movement = movementFromRoadLink(roadLink.type)
    for (const laneLink of roadLink.laneLinks ?? []) {
      laneLinks.set(laneLink.id, { laneLink, movement })
      const key = approachLaneKey(roadLink.fromRoadId, laneLink.startLaneIndex)
      const movements = approachMovements.get(key) ?? []
      movements.push(movement)
      approachMovements.set(key, movements)
    }
  }

  const context = {
    coordinates: new IntersectionCoordinateSystem(intersection),
    connectedRoads,
    laneLinks,
    approachMovements,
  }
  contexts.set(intersectionId, context)
  return context
}
...
export function createLocalVehiclePoses(
  ...
  const context = getPoseContext(roadnet, intersectionId)
  if (!context) return []
  const poses: LocalVehiclePose[] = []

  for (const vehicle of vehicles) {
    if (vehicle.drivableType === 'lane_link' && vehicle.drivableId) {
      const match = context.laneLinks.get(vehicle.drivableId)
      if (!match || !context.connectedRoads.has(vehicle.roadId)) continue

      const sample = samplePolylineAtDistance(match.laneLink.points, vehicle.distance ?? 0)
      if (!sample) continue
      const position = context.coordinates.cityFlowPointToThree(sample.point, height)
    ...
    const road = context.connectedRoads.get(vehicle.roadId)
    if (!road) continue

    const position = context.coordinates.cityFlowPointToThree(vehicle, height)
    if (Math.hypot(position.x, position.z) > visibleRadius) continue
    ...
    poses.push({
      ...
      position,
      rotationY: Math.atan2(direction.x, direction.z),
      movement: context.approachMovements.get(approachLaneKey(vehicle.roadId, laneIndex))?.length === 1
        ? context.approachMovements.get(approachLaneKey(vehicle.roadId, laneIndex))![0]!
        : 'straight',
      onLaneLink: false,
    })
  }
