...
const scenes = [
  ...
  ], p("direct_answer", "缺少必填的 intersectionId，不能编造参数；需要用户补充具体路口 ID 后再查询。")),
  ...
  ], p("direct_answer", "缺少必填的 roadId，不能编造参数；需要用户补充具体道路 ID 后再查询。")),
  ...
  ], p("direct_answer", "缺少必填的 endIntersection，不能调用草案工具或编造终点；需要用户补充终点路口。")),
  ...
  ], p("direct_answer", "缺少必填的 startIntersection，不能调用草案工具或编造起点；需要用户补充起点路口。")),
...
function validateScene(scene, allQuestions) {
  ...
  for (const toolCall of calls) {
    ...
    for (const requiredGroup of spec.requiredAny ?? []) {
      const hasRequiredValue = requiredGroup.some((argName) => {
        const value = toolCall.arguments[argName];
        return value !== undefined && value !== null && String(value).trim() !== "";
      });
      if (!hasRequiredValue) {
        errors.push(`${scene.id}: ${toolCall.toolName} 至少需要参数之一 ${requiredGroup.join("/")}`);
      }
    }
...
const plannerSourcePath = path.join(ROOT, "backend", "src", "main", "java", "com", "traffic", "agent", "orchestrator", "AgentIntentClassifier.java");
const plannerSourceCode = fs.readFileSync(plannerSourcePath, "utf8");
const plannerPromptMatch = plannerSourceCode.match(/private String plannerSystemPrompt\(\) \{\s*return """\r?\n([\s\S]*?)\r?\n\s*""";/u);
if (!plannerPromptMatch) {
  allErrors.push("无法从 AgentIntentClassifier 提取 plannerSystemPrompt");
} else {
  const sourceLines = plannerPromptMatch[1].replaceAll("\r\n", "\n").split("\n");
  ...
  if (productionPrompt !== SYSTEM_PROMPT.trim()) {
    allErrors.push("系统提示词与 AgentIntentClassifier 不完全一致");
  }
}
...
const report = {
  ...
  intentCounts,
  toolCounts,
  promptSynchronized: !allErrors.some((error) => error.startsWith("系统提示词与") || error.startsWith("无法从 AgentIntentClassifier")),
  errors: allErrors,
};
