"""西安20路口路径规划 + CityFlow roadnet生成"""
import json, time, urllib.request, urllib.parse, os

AMAP_KEY = "4ab584658b1cdc916345e3c20bc15add"
SRC = r"c:\Users\Cxaiorui\Downloads\shanghai_adjusted (4).json"

with open(SRC, encoding="utf-8") as f:
    src = json.load(f)

its = src["intersections"]  # already exactly 20
...
for it_idx, it in enumerate(its):
    r,c = it["row"], it["col"]
    cl = [i for i,rd in enumerate(roads) if (rd["fc"]==c and rd["fr"]==r) or (rd["tc"]==c and rd["tr"]==r)]
    cf_its[it_idx]["roads"] = [rid_map[roads[i]["id"]] for i in cl]
...
os.makedirs("sim-python/data/xian", exist_ok=True)
with open("sim-python/data/xian/roadnet.json","w",encoding="utf-8") as f: json.dump(rdn,f,ensure_ascii=False,indent=2)
flows=[{"vehicle":{"length":5,"width":2,"maxPosAcc":2,"maxNegAcc":4.5,"usualPosAcc":2,"usualNegAcc":4.5,"minGap":2.5,"maxSpeed":16.67,"headwayTime":1.5},"route":[rid_map[rd["id"]]],"interval":3.0,"startTime":0,"endTime":-1} for rd in roads]
with open("sim-python/data/xian/flow.json","w",encoding="utf-8") as f: json.dump(flows,f,ensure_ascii=False,indent=2)

print("Done: {} its, {} roads, {} flows".format(len(cf_its),len(cf_roads),len(flows)))
