...
class EVPriorityService:
    ...
    def step(self, sid: str, engine: Any, sim_time: float
        ...
        for ev_id, ev_session in self.ev_sessions[sid].items():
            ...
            if not vehicle_info:
                arrived = False
                if hasattr(engine, "has_vehicle_arrived") and ev_session.cf_vehicle_id:
                    try:
                        arrived = bool(engine.has_vehicle_arrived(ev_session.cf_vehicle_id))
                    except Exception:
                        arrived = False
                if arrived:
    ...
    # Prefer simulator-native phase duration control when the engine supports it.
        ...
        if decision == 'green_extend':
            ...
            if hasattr(engine, 'build_ev_priority_program'):
                ...
                print(f'  [PAPER] {decision.upper()} {inter_id}: '
                      ...
                      f'(EV phase applied and extended)')
